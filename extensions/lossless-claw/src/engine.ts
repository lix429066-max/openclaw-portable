...
export class LcmContextEngine implements ContextEngine {
  ...
  private async resolveSummarize(params: {
    ...
    try {
      ...
      const provider = String(lp.provider ?? "unknown");
      const model = String(lp.model ?? "unknown");
      console.error(
        `[lcm] resolveSummarize FAILED (provider=${provider}, model=${model}):`,
        err instanceof Error ? err.message : err,
        "| FALLING BACK TO EMERGENCY TRUNCATION",
      );
    }
    console.error(`[lcm] resolveSummarize: using emergency truncation fallback (text slicing only, no LLM summarization)`);
