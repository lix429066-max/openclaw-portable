...
export interface CompactionConfig {
  ...
  timezone?: string;
  /**
   * Context collapse threshold as fraction of budget (default 0.90).
   * When context exceeds this threshold, compaction uses aggressive mode
   * to perform a deep collapse — generating much tighter summaries to
   * rapidly reclaim headroom in emergency near-limit situations.
   * Mirrors Claude Code's CONTEXT_COLLAPSE emergency recovery strategy.
   */
...
export class CompactionEngine {
  ...
  async compactFullSweep(input: {
    ...
    const tokensBefore = await this.summaryStore.getContextTokenCount(conversationId);
    const threshold = Math.floor(this.config.contextThreshold * tokenBudget);
    const collapseThreshold = Math.floor(
      (this.config.collapseThreshold ?? 0.9) * tokenBudget,
    );
    const inCollapseZone = tokensBefore >= collapseThreshold;
    const leafTrigger = await this.evaluateLeafTrigger(conversationId);
  ...
  private async summarizeWithEscalation(params: {
    ...
    summarize: CompactionSummarizeFn;
    options?: CompactionSummarizeOptions;
    /** Skip normal mode, start directly with aggressive (for collapse zone). */
    ...
    if (params.forceAggressive) {
      summaryText = await params.summarize(sourceText, true, params.options);
      level = "aggressive";
    } else {
      summaryText = await params.summarize(sourceText, false, params.options);
      level = "normal";
    }
